...
Depois do ajuste, os boxplots passaram a usar `points=False`. Assim, os gráficos mostram apenas:
...
Essa decisão melhora a clareza visual da apresentação. O trade-off é que os valores individuais deixam de aparecer no gráfico, mas a leitura da distribuição de cada variável fica mais limpa.

## Registro do ajuste da tabela de métricas do notebook supervisionado

Em 24_05_2026, o notebook `notebooks/01_ia_supervisionada_credito.ipynb` foi ajustado para simplificar a visualização do relatório de classificação do modelo de crédito.
...
A tabela final passa a exibir somente:

- linhas das classes de negócio, `aprovado` e `reprovado`
...
### Decisão sobre o rótulo `reprovado`

Fato: a base atual usa o rótulo `negado` na coluna `decisao_credito`.

Inferência: o pedido de exibir `reprovado` se refere à mesma classe de negócio representada por `negado`, apenas com outro nome mais natural para a apresentação.

Opinião técnica: a melhor escolha foi manter o rótulo original nos dados e no treinamento, mas renomear `negado` para `reprovado` apenas na tabela final. Isso evita alterar a base, reduz risco de quebrar validações existentes e atende ao formato visual solicitado.
...
Também foram ajustadas referências soltas a `base_credito` nas células executáveis do notebook supervisionado. Como a base passou a ser lida em `df = pd.read_csv('../data/dados_clientes.csv')`, manter chamadas a `base_credito` deixava o notebook dependente de uma variável antiga no kernel.
